...
> 如果PATH环境变量没配，需要 `export PATH=$PATH:$GOPATH/bin`，然后运行 `source ~/.bash_profile` 或 `source ~/.zshrc` 来使更改生效。
...
与`Form字段`的唯一区别在于：因为`FormValue`方法在需要时会自动调用`ParseForm`方法或者`ParseMultipartForm`方法，所以用户在执行`FormValue`方法之前，不需要手动调用上面提到的两个语法分析方法
...
`Must函数`可以包裹起一个函数，被包裹的函数会返回一个指向模板的指针和一个错误，如果这个错误不是nil，那么Must函数将产生一个`panic`。
